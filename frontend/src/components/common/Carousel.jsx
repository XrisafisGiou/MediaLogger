...
function getVisibleCount(slidesPerView, isMediumScreen) {
  if (typeof slidesPerView === "number") return slidesPerView;
  return isMediumScreen
    ? (slidesPerView.md ?? slidesPerView.base ?? 1)
...
export default function Carousel({
  ...
  useEffect(() => {
    ...
    const updateScreenSize = (event) => setIsMediumScreen(event.matches);
  ...
  const visibleCount = Math.min(
    getVisibleCount(slidesPerView, isMediumScreen),
    items.length,
  );
  ...
  function previous() {
    setActiveIndex(safeIndex <= 0 ? lastIndex : safeIndex - 1);
  }

  function next() {
    setActiveIndex(safeIndex >= lastIndex ? 0 : safeIndex + 1);
  }
  ...
  return (
    <section className={className} aria-label={ariaLabel}>
      ...
      <div className="relative">
        ...
        <div className={canNavigate ? "mx-10 overflow-hidden" : "overflow-hidden"}>
          <div
            className="flex gap-4 transition-transform duration-500 ease-out"
            style={{
              transform: `translateX(calc(-${(safeIndex * 100) / visibleCount}% - ${(safeIndex * gap) / visibleCount}px))`,
            }}
          >
